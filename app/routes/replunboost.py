...
@app.route("/api/v1/me/replunboost", methods=['POST'])
def unboostRepl():
	if request.method == "POST":
		...
		if "repl:unboost" not in scopes:
			return abort(403)

		sid = tokens.get(token)['sid']

		json_data = {
		    'query': """mutation unboostRepl ($id: String!) {
 unboostRepl (replId: $id) {
  ...on UserError {
    message
  }
  ...on ReplBoost {
    id
		...
		return r.json()['data']['unboostRepl']