...
@app.route("/api/v1/me/replunpublish", methods=['POST'])
def unpublishRepl():
	if request.method == "POST":
		...
		if "repl:unpublish" not in scopes:
			return abort(403)

		sid = tokens.get(token)['sid']

		json_data = {
		    'query': """mutation unpublishRepl ($input: UnpublishReplInput!) {
 unpublishRepl (input: $input) {
      ...
      message
		...
		return r.json()['data']['unpublishRepl']