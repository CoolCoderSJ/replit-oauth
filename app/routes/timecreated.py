...
@app.route("/api/v1/me/timeCreated", methods=['POST'])
def timeCreated():
	if request.method == "POST":
		...
		if "user:timeCreated" not in scopes:
			return abort(403)

		sid = tokens.get(token)['sid']

		json_data = {
		    'query': 'query current {currentUser {timeCreated}}',
		}
		...
		return r.json()['data']['currentUser']['timeCreated']