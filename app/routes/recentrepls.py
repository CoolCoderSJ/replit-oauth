...
@app.route("/api/v1/me/recentrepls", methods=['POST'])
def recentrepls():
	if request.method == "POST":
		...
		if "user:recentrepls" not in scopes:
			return abort(403)

		sid = tokens.get(token)['sid']

		json_data = {
		    'query': 'query recentrepls ($count:Int) {recentRepls(count:$count) {id, title, description, language, isPrivate }}',
		    'variables': {
		        'count': form['count'],
		    },
		}
		...
		return jsonify(r.json()['data']['recentRepls'])