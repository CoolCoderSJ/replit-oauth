...
@app.route("/api/v1/me/replpublish", methods=['POST'])
def publishRepl():
	if request.method == "POST":
		...
		if "repl:publish" not in scopes:
			return abort(403)

		sid = tokens.get(token)['sid']

		json_data = {
		    'query': """mutation publishRepl ($input: PublishReplInput!) {
 publishRepl (input: $input) {
      ...
      message
		...
		return r.json()['data']['publishRepl']