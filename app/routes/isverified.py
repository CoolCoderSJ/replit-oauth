...
@app.route("/api/v1/me/isVerified", methods=['POST'])
def isVerified():
	if request.method == "POST":
		...
		if "user:isVerified" not in scopes:
			return abort(403)

		sid = tokens.get(token)['sid']

		json_data = {
		    'query': 'query current {currentUser {isVerified}}',
		}
		...
		return r.json()['data']['currentUser']['isVerified']