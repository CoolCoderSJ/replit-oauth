...
@app.route("/api/v1/me/repls", methods=['POST'])
def repls():
	if request.method == "POST":
		...
		if "user:repls" not in scopes:
			return abort(403)

		sid = tokens.get(token)['sid']

		json_data = {
		    'query': 'query user($username:String!){user:userByUsername(username:$username){publicRepls(count:9999999,showUnnamed:true){items{id,title,language,isPrivate}}}}',
			"variables": {
		...
		return jsonify(r.json()['data']['repls'])