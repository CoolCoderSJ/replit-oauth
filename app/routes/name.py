...
@app.route("/api/v1/me/fullName", methods=['POST'])
def fullName():
	if request.method == "POST":
		...
		if "user:fullName" not in scopes:
			return abort(403)

		sid = tokens.get(token)['sid']

		json_data = {
		    'query': 'query current {currentUser {fullName}}',
		}
		...
		return r.json()['data']['currentUser']['fullName']